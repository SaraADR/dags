...
def fetch_minio_image(identifier, s3_client, bucket_name):
    try:
        ...
        logging.info(f"Image {identifier} fetched and encoded successfully.")

        return image_base64

    except Exception as e:
        logging.error(f"Error fetching image {identifier}: {e}")
        return None
...
def generate_xml(**kwargs):
    logging.info("Starting XML generation.")

    xml_encoded = []
    
    algoritm_result = kwargs['dag_run'].conf.get('json')
    logging.info(f"JSON content loaded: {algoritm_result}")

    executionResources = algoritm_result['executionResources']
    logging.info(f"Execution Resources found: {len(executionResources)} resources")
    ...
    logging.info(f"BBOX coordinates: {bbox} with reference system {coordinate_system}")
    ...
    logging.info("Calling convertir_coords.")
    ...
    for resource in executionResources:
        if resource['output'] == False:
            logging.info("Skipping non-output resource.")
            continue

        if not re.search(r'\.tif$', resource['path'], re.IGNORECASE):
            logging.info("Skipping non-TIFF resource.")
            continue
        ...
        logging.info(f"Processing resource with identifier={identifier} and resolution={spatial_resolution}")

        # Fetch the image from MinIO
        image_base64 = fetch_minio_image(identifier, s3_client, bucket_name)
        if image_base64 is None:
            logging.warning(f"No image found for identifier {identifier}, skipping image in XML.")
        
        wms_link = algoritm_result['executionResources'][0]['path']  # Dynamic WMS link
        ...
        tree = creador_xml_metadata(
            ...
            layer_description="Layer description here",
            thumbnail_image_base64=image_base64  # Pass the image data to XML generator
        )
